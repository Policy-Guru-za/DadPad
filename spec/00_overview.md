...
| `14_portrait-only-orientation-lock` | Lock DadPad to standard upright portrait orientation only | In Progress | Codex | `13_chip-layout-and-ios-icon-refresh` | Remove landscape and upside-down orientation support from the native iOS configuration so the app never rotates into an unusable layout |
