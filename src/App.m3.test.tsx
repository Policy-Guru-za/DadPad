...
describe("DadPad app", () => {
  ...
  it("blocks the editor, actions, and settings while the offline overlay is visible", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        (screen.getByRole("button", { name: "Polish" }) as HTMLButtonElement).disabled,
      ).toBe(true);
  ...
  it("renders the Warm Sand DadPad shell with the remapped bottom action grid", async () => {
    ...
    expect(getActionBarLabels()).toEqual(["Polish", "Clear", "Settings", "Notes", "Gmail"]);
  ...
  it("keeps Polish styled as the primary action while preserving the secondary/system treatments", async () => {
    await renderApp();
